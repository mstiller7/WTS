...
public abstract class SlotGUI {
	...
	protected void renderSelected(ScaledResolution resolution, int mouseX, int mouseY, ItemStack stack, int renderSlotX,
                                int renderSlotY) {
		...
		GuiUtils.drawHoveringText(stack.getTooltip(MC.player, MC.gameSettings.advancedItemTooltips), mouseX, mouseY,
      resolution.getScaledWidth(), resolution.getScaledHeight(), -1, MC.fontRendererObj);
	}
	...
	protected void drawTexturedRectangle(int x, int y, int textureX, int textureY, int width, int height,
